...
### Receipt fixture cleanup (2026-09-23)

Starting from pre-commit HEAD `172afed23ec3251c7d32af52dac5b65f7decffac`, the previously recorded receipt test error was reproduced independently: `test_receipt_email_always_displays_invoice_portal_action` errored because its `SimpleNamespace` invoice fixture lacked `.get()`. The fixture now uses `frappe._dict`, matching the invoice shape already used in `test_stripe_receipt_labels.py`; its assertions for the Parent Portal action text, URL, and `include_portal_link=True` remain unchanged. This is a test-only cleanup; production code is unchanged.

Python 3.11.15 ran from `/Users/ranxu/Documents/Project/frappe-bench/env/bin/python` with this checkout on `PYTHONPATH` and `PYTHONDONTWRITEBYTECODE=1`. The target test passed **1/1**, the full `test_parent_invoice_portal_actions` module passed **3/3**, and the exact 13-module command above passed **116/116** (`Ran 116 tests`, `OK`). No Frappe site was initialized or connected, and no database transaction, email, payment, or deployment was exercised.
