...
class QASPAYGBooking(Document):
    ...
    def validate(self):
        ...
        if creating:
            ...
            if not flags or flags.get("payg_mutation_token") is not QASPAYGBooking._SERVICE_MUTATION_TOKEN:
                frappe.throw("PAYG booking changes must use a locked service command")
        ...
        if creating:
            ...
            return
        # The mutation service already checked linked rows under the global lock
        # order. Document.save locks Booking before validate, so a controller
        # Student/Card locking read here would invert that order.
