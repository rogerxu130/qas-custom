...
class TestStoreOrderAdminNotification(TestCase):
    ...
    def test_email_has_order_details_and_escapes_user_content(self):
        ...
        with patch.object(frappe, 'db', SimpleNamespace(get_value=Mock(return_value='Parent & Child'))), patch.object(service, 'get_url', side_effect=lambda path:'https://system.example'+path):
            html=service.new_order_email_content(doc)
        for expected in ['O1','Parent &amp; Child','Campus &lt;One&gt;','A$158.00','/app/store-order/O1','&lt;script&gt;']:
